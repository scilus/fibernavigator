function T=msfm2d(F, SourcePoints, usesecond, usecross)
    ...
    for k=1:4,
        ...
        if((i>0)&&(j>0)&&(i<=size(F,1))&&(j<=size(F,2))&&(Frozen(i,j)==0))
            ...
            for nx=-2:2
                for ny=-2:2
                    in=i+nx; jn=j+ny;
                    if((in>0)&&(jn>0)&&(in<=size(F,1))&&(jn<=size(F,2))&&(Frozen(in,jn)==1))
            ...
            Coeff = [0 0 -1/(F(i,j)^2+eps)];
            for t=1:2;
                switch(Order(t))
                    case 1,
                        Coeff=Coeff+[1 -2*Tm(t) Tm(t)^2];
                    case 2,
                        Coeff=Coeff+[1 -2*Tm2(t) Tm2(t)^2]*(9/4);
                end
            end
            ...
            if(usecross)
                ...
                for t=3:4;
                    switch(Order(t))
                        ...
                        case 2,
                            Coeff=Coeff+0.5*[1 -2*Tm2(t) Tm2(t)^2]*(9/4);
                    end
                end
