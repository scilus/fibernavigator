    ...
    m_pointArray( pointArray )
{
    ...
    if( useThreads )
    {
        Logger::getInstance()->print( wxT( "Building kd tree..." ), LOGLEVEL_MESSAGE );
...
KdTreeThread::KdTreeThread(float *pointArray, std::vector<wxUint32>* tree, int left, int right, int axis )
:   m_tree(tree),
    m_pointArray(pointArray),
    m_left(left),
    m_right(right),
