...
private:
    ...
    void             getODFpoints               ( FMatrix &i_phiThetaDirection, vector < float > &o_deformedMeshPts );    
    complex< float > getSphericalHarmonic       ( int i_l, int i_m, float i_theta, float i_phi );
    void             getSphericalHarmonicMatrix ( const vector< float > &i_meshPts, FMatrix &o_phiThetaDirection, FMatrix &o_shMatrix );
