...
//     }
     ...
     * Only tested on the visContest fibers
     *
...
void Fibers::colorWithMinDistance( float* i_colorData )
{
    ...
    for(int i = 0; i < getLineCount(); ++i)
    {
        ...
        for(int j = 0; j < NbPointsInLine; ++j)
        {
            ...
            //i_colorData[(Index + j)*3]     = 1;
            //i_colorData[(Index + j)*3 + 1] = 1;
            //i_colorData[(Index + j)*3 + 2] = 0;
        }
    }
}
...
void Fibers::generateFiberVolume()
{
    ...
    m_dh->m_mainFrame->m_listCtrl->SetItemState(
        0,
...
void Fibers::saveDMRI( wxString i_fileName )
{
    ...
    int min = m_dh->m_mainFrame->m_sliderFibersFilterMin->GetValue();
	int max = m_dh->m_mainFrame->m_sliderFibersFilterMax->GetValue();
	int subSampling = m_dh->m_mainFrame->m_sliderFibersSampling->GetValue();
	int maxSubSampling = m_dh->m_mainFrame->m_sliderFibersSampling->GetMax()+1;
    int nbrlines=0;
