...
bool Fibers::load( wxString i_filename )
{
   if( i_filename.AfterLast( '.' ) == _T( "fib" ) ) {
      if (loadVTK( i_filename )){
			...
			return loadDmriAscii(i_filename);  
		}
   }
   
   if( i_filename.AfterLast( '.' ) == _T( "bundlesdata" ) )
      return loadPTK( i_filename );

   if( i_filename.AfterLast( '.' ) == _T( "Bfloat" ) )
      return loadCamino( i_filename );
...
// Comment
///////////////////////////////////////////////////////////////////////////
bool Fibers::loadDmriAscii(wxString i_fileName)
{
	FILE *l_file;	
...
void Fibers::updateLinesShown()
{
    ...
    if( l_boxWasUpdated )
        m_dh->m_mainFrame->SetFiberInfoGridValues( m_dh->m_lastSelectedObject );
}
...
vector< bool > Fibers::getLinesShown( SelectionObject* i_selectionObject )
{
    ...
    else
    {
        for( int i = 0; i < m_countPoints; ++i )
        {
            int l_x     = wxMin( m_dh->m_columns -1, wxMax( 0, (int)m_pointArray[i * 3 ]    ) ) / m_dh->m_xVoxel;
            int l_y     = wxMin( m_dh->m_rows    -1, wxMax( 0, (int)m_pointArray[i * 3 + 1] ) ) / m_dh->m_yVoxel;
            int l_z     = wxMin( m_dh->m_frames  -1, wxMax( 0, (int)m_pointArray[i * 3 + 2] ) ) / m_dh->m_zVoxel;
            int l_index = l_x + l_y * m_dh->m_columns + l_z * m_dh->m_rows * m_dh->m_columns;

            if( ( i_selectionObject->m_sourceAnatomy->at( l_index ) - i_selectionObject->getThreshold() ) > 0.01f )
...
#if 0 // FIXME : Will never hit of this! What's the point of this?
    if ( m_dh->m_useVBO )
    {
        glBindBuffer( GL_ARRAY_BUFFER, m_bufferObjects[1] );
        l_colors = (GLfloat *) glMapBuffer( GL_ARRAY_BUFFER, GL_READ_ONLY );
       //glUnmapBuffer( GL_ARRAY_BUFFER );

        glBindBuffer( GL_ARRAY_BUFFER, m_bufferObjects[2] );
        l_normals = (GLfloat *) glMapBuffer( GL_ARRAY_BUFFER, GL_READ_ONLY );
        //glUnmapBuffer( GL_ARRAY_BUFFER );
    }
    else
    {
...
void Fibers::drawSortedLines()
{
    ...
    // cout << "nb snipplets total: " << l_nbSnipplets << endl;
...
#ifdef __VERBOSE__
    cout << "projection l_matrix:" << endl
    << "(  " << l_matrix[0] << " # " << l_matrix[4] << " # " << l_matrix[8] << " # " << l_matrix[12] << ")" << endl
    << "(  " << l_matrix[1] << " # " << l_matrix[5] << " # " << l_matrix[9] << " # " << l_matrix[13] << ")" << endl
    << "(  " << l_matrix[2] << " # " << l_matrix[6] << " # " << l_matrix[10] << " # " << l_matrix[14] << ")" << endl
    << "(  " << l_matrix[3] << " # " << l_matrix[7] << " # " << l_matrix[11] << " # " << l_matrix[15] << ")" << endl;
#endif

#if 0 // FIXME : Will never hit of this! What's the point of this?
    glGetFloatv( GL_MODELVIEW_MATRIX, l_matrix );
#ifdef __VERBOSE__
    cout << "modelview l_matrix:" << endl
    << "(  " << l_matrix[0] << " # " << l_matrix[4] << " # " << l_matrix[8] << " # " << l_matrix[12] << ")" << endl
    << "(  " << l_matrix[1] << " # " << l_matrix[5] << " # " << l_matrix[9] << " # " << l_matrix[13] << ")" << endl
    << "(  " << l_matrix[2] << " # " << l_matrix[6] << " # " << l_matrix[10] << " # " << l_matrix[14] << ")" << endl
    << "(  " << l_matrix[3] << " # " << l_matrix[7] << " # " << l_matrix[11] << " # " << l_matrix[15] << ")" << endl;
...
void Fibers::switchNormals( bool i_positive )
{
    ...
    {
        glBindBuffer( GL_ARRAY_BUFFER, m_bufferObjects[2] );
        l_normals = (float *) glMapBuffer( GL_ARRAY_BUFFER, GL_READ_WRITE );
    }
    else
    {
        l_normals = &m_normalArray[0];
