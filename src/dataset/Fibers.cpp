...
Fibers::~Fibers()
{
    m_dh->printDebug( _T( "executing fibers destructor" ), MESSAGELEVEL );
...
{
    stringstream ss;
    m_dh->printDebug( wxT( "Start loading TRK file..." ), MESSAGELEVEL );
    wxFile dataFile;
    ...
    ss.str( "" );
    ss << "Type: " << idString;
    m_dh->printDebug( wxString( ss.str().c_str(), wxConvUTF8 ), MESSAGELEVEL );
    ...
    ////
    m_dh->printDebug( wxT( "Setting data in right format for the navigator..." ), MESSAGELEVEL );
    ...
    m_dh->printDebug( wxT( "End loading TRK file" ), MESSAGELEVEL );
...
{
    m_dh->printDebug( _T( "start loading Camino file" ), MESSAGELEVEL );
    wxFile dataFile;
    wxFileOffset nSize = 0;
    ...
    m_dh->printDebug( _T( "move vertices" ), MESSAGELEVEL );
    ...
    m_dh->printDebug( _T( "read all" ), MESSAGELEVEL );
    delete[] pBuffer;
    pBuffer = NULL;
...
{
    m_dh->printDebug( _T( "start loading MRtrix file" ), MESSAGELEVEL );
    wxFile dataFile;
    long int nSize = 0;
    ...
    m_dh->printDebug( wxT( "End loading TCK file" ), MESSAGELEVEL );
    createColorArray( false );
    m_type = FIBERS;
    m_fullPath = filename;
    //m_pKdTree = new KdTree( m_countPoints, &m_pointArray[0], m_dh );
...
{
    m_dh->printDebug( _T( "start loading PTK file" ), MESSAGELEVEL );
    wxFile dataFile;
    wxFileOffset nSize = 0;
    int pc = 0;
...
{
    m_dh->printDebug( _T( "start loading VTK file" ), MESSAGELEVEL );
    wxFile dataFile;
    wxFileOffset nSize = 0;
    ...
    m_dh->printDebug( wxString::Format( _T( "loading %d points and %d lines." ), countPoints, countLines ), MESSAGELEVEL );
...
{
    m_dh->printDebug( _T( "toggle Endianess" ), MESSAGELEVEL );
    wxUint8 temp = 0;
...
{
    m_dh->printDebug( _T( "calculate line pointers" ), MESSAGELEVEL );
    int pc = 0;
    int lc = 0;
    int tc = 0;
...
{
    m_dh->printDebug( _T( "create color arrays" ), MESSAGELEVEL );
...
{
    m_dh->printDebug( _T( "reset color arrays" ), MESSAGELEVEL );
    float *pColorData =  NULL;
...
void Fibers::initializeBuffer()
{
    ...
    {
        m_dh->printDebug( _T( "Not enough memory on your gfx card. Using vertex arrays." ),            ERRORLEVEL );
        m_dh->printDebug( _T( "This shouldn't concern you. Perfomance just will be slightly worse." ), ERRORLEVEL );
        m_dh->printDebug( _T( "Get a better graphics card if you want more juice." ),                  ERRORLEVEL );
        glDeleteBuffers( 3, m_bufferObjects );
    }
}
...
void Fibers::flipAxis( AxisType i_axe )
{
    ...
    switch ( i_axe )
    {
        ...
        case Z_AXIS:
            ...
            break;
        default:
            m_dh->printDebug( _T("Cannot flip fibers. The specified axis is undefined"), ERRORLEVEL );
