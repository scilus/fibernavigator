...
void ODFs::drawGlyph( int i_zVoxel, int i_yVoxel, int i_xVoxel, AxisType i_axis )
{
    ...
    //flip ODF to fiberNavigator world
...
// matrix containing the phi and theta directions
//
...
///////////////////////////////////////////////////////////////////////////
void ODFs::getSphericalHarmonicMatrix( const vector< float > &i_meshPts, FMatrix &o_phiThetaDirection, FMatrix &o_shMatrix )
{
...
complex< float > ODFs::getSphericalHarmonic( int i_l, int i_m, float i_theta, float i_phi ) 
{
    ...
    float l_factor = sqrt( ( (float)( 2 * i_l + 1 ) / ( 4.0f * M_PI ) ) * ( Helper::getFactorial( i_l - l_absm ) /
