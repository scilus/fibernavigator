...
{      
    ...
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->bind();
    
    glBindTexture( GL_TEXTURE_1D, m_textureId );

    // This is the color look up table texture.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniSampler( "clut",          0                         );
    // This is the alpha level of the tensors.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniFloat  ( "alpha",         DatasetInfo::m_alpha      );
    // This is the value for the lighting attenuation.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniFloat  ( "attenuation",   m_lighAttenuation         );
    // This is the value for the light direction.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUni3Float ( "lightPosition", m_lightPosition           );
    // This is the brightness level of the tensors.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniFloat  ( "brightness",    DatasetInfo::m_brightness );
    // If m_colorWithPosition is true then the glyph will be colored with the position of the vertex.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniInt    ( "colorWithPos", (GLint)m_colorWithPosition );
 
    Glyph::draw();

    // Disable the tensor color shader.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->release();
}
...
void Tensors::drawGlyph( int i_zVoxel, int i_yVoxel, int i_xVoxel, AxisType i_axis )
{
    ...
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUni3Float( "offset", l_offset );

    // Lets set the color to draw this tensor.
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setAttribFloat( "color", l_tensorFA );
    ...
    DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUni3Float(   "axisFlip",    
                                                                        l_flippedAxes);
    ...
    if (getDisplayShape()== NORMAL || getDisplayShape()== SPHERE ){
        if (isDisplayShape(SPHERE)){
            ...
            DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniMatrix3f( "tensorMatrix", l_noDeformMatrix );
        } else {
            // Lets set the matrix of this tensor so the shader will be able to deform it properly.
            DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniMatrix3f( "tensorMatrix", m_tensorsMatrix[l_tensorNumber] );
        }
        ...
        DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniInt( "displayControl", 0 );
        ...
        // Lets set the radius modifier.
        DatasetInfo::m_dh->m_shaderHelper->m_pTensorsShader->setUniInt( "displayControl", 1 );
        // Depending on the orientation of the glyph we do a front or back face culling.
