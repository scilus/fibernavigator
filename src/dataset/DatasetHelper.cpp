...
DatasetHelper::DatasetHelper( MainFrame* mf ) :
	...
	m_displayFibersInfo        ( false        ),
    m_displayMeanFiber         ( false        ),
...
bool DatasetHelper::load( const int i_index )
{
    ...
    wxString l_wildcard         = wxT( "*.*|*.*|Nifti (*.nii.gz)|*.nii.gz|Mesh files (*.mesh)|*.mesh|Mesh files (*.surf)|*.surf|Mesh files (*.dip)|*.dip|Fibers VTK (*.fib)|*.fib|Fibers PTK (*.bundlesdata)|*.bundlesdata|Scene Files (*.scn)|*.scn|Tensor files (*.nii.gz)|*.nii.gz|ODF files (*.nii.gz)|*.nii.gz" );
...
///////////////////////////////////////////////////////////////////////////
// COMMENT
bool DatasetHelper::load( wxString i_fileName, const int i_index, const float i_threshold, const bool i_active, const bool i_showFS, const bool i_useTex, const float i_alpha )
{
    ...
    {
        if( i_index == 8 ) // This mean that the user clicked on the Load Tensors button
        {
            if( ! m_anatomyLoaded )
            {
            ...
            Tensors* l_tensors = new Tensors( this );

            if( l_tensors->load( i_fileName ) )
            {
                l_tensors->setThreshold( i_threshold );
                l_tensors->setShow     ( i_active );
                l_tensors->setShowFS   ( i_showFS );
                l_tensors->setuseTex   ( i_useTex );

                finishLoading( l_tensors );
        ...
        else if( i_index == 9 ) // This mean that the user clicked on the Load ODFs button.
        {
            if( ! m_anatomyLoaded )
            {
                m_lastError = wxT( "no anatomy file loaded" );
            ...
            ODFs* l_ODFs = new ODFs( this );

            if( l_ODFs->load( i_fileName ) )
            {
                l_ODFs->setThreshold( i_threshold );
                l_ODFs->setShow     ( i_active );
                l_ODFs->setShowFS   ( i_showFS );
                l_ODFs->setuseTex   ( i_useTex );

                finishLoading( l_ODFs );
                m_ODFsLoaded = true;
                return true;
            }
            else
        ...
        {    
            Anatomy* l_anatomy = new Anatomy( this );

            if( l_anatomy->load( i_fileName ) )
            {
                l_anatomy->setThreshold( i_threshold );
                l_anatomy->setAlpha    ( i_alpha );
                l_anatomy->setShow     ( i_active );
                l_anatomy->setShowFS   ( i_showFS );
                l_anatomy->setuseTex   ( i_useTex );
                finishLoading( l_anatomy );

                m_floatDataset = l_anatomy->getFloatDataset();
                return true;
            }
            else
            {
    ...
    else if( l_ext == _T( "fib" ) || l_ext == _T( "bundlesdata" ) || l_ext == _T( "Bfloat" ) )
    {
        ...
        if( l_fibers->load( i_fileName ) )
        {
			...
			l_fibers->setFibersLength();
			m_mainFrame->m_sliderFibersFilterMin->SetRange((int)l_fibers->getMinFibersLength(), (int)l_fibers->getMaxFibersLength()+1);
			m_mainFrame->m_sliderFibersFilterMin->SetValue((int)l_fibers->getMinFibersLength());
			m_mainFrame->m_sliderFibersFilterMin->Enable();
			m_mainFrame->m_sliderFibersFilterMax->SetRange((int)l_fibers->getMinFibersLength(), (int)l_fibers->getMaxFibersLength()+1);
			m_mainFrame->m_sliderFibersFilterMax->SetValue((int)l_fibers->getMaxFibersLength()+1);
			m_mainFrame->m_sliderFibersFilterMax->Enable();
			m_mainFrame->m_sliderFibersSampling->Enable();
            return true;
        }
        return false;
    }
...
void DatasetHelper::createIsoSurface()
{
    ...
    CIsoSurface* isosurf = new CIsoSurface( this, l_anatomy ); //->getFloatDataset());
...
void DatasetHelper::createDistanceMapAndIso()
{
    ...
    if( isosurf->IsSurfaceValid() )
    {
        ...
        m_mainFrame->m_listCtrl->SetItem( 0, 1, isosurf->getName() );
        m_mainFrame->m_listCtrl->SetItem( 0, 2, wxT( "0.10" ) );
        m_mainFrame->m_listCtrl->SetItem( 0, 3, wxT( "" ), 1 );
        m_mainFrame->m_listCtrl->SetItemData( 0, (long) isosurf );
