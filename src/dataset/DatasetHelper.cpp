...
bool DatasetHelper::load( const int index )
{
    ...
    wxString
            wildcard =
                    wxT("*.*|*.*|Nifti (*.nii)|*.nii*|Mesh files (*.mesh)|*.mesh| Mesh files (*.surf)|*.surf|Mesh files (*.dip)|*.dip| Fibers VTK (*.fib)|*.fib|Fibers PTK (*.bundlesdata)|*.bundlesdata| Scene Files (*.scn)|*.scn");
    wxString defaultDir = wxEmptyString;
    wxString defaultFilename = wxEmptyString;
    wxFileDialog dialog( mainFrame, caption, defaultDir, defaultFilename, wildcard, wxOPEN | wxFD_MULTIPLE );
...
bool DatasetHelper::load( wxString filename, const float threshold, const bool active, const bool showFS,
        const bool useTex, const float alpha )
{
...
void DatasetHelper::finishLoading( DatasetInfo *info )
{
    ...
    if ( mainFrame->m_listCtrl->GetItemCount() == 1 )
    {
        ...
        updateView( mainFrame->m_xSlider->GetValue(), mainFrame->m_ySlider->GetValue(),
            ...
            while ( datasetnode )
            {
                ...
                while ( nodes )
                {
                    if ( nodes->GetName() == _T("status") )
                    {
                        ...
                        ( nodes->GetPropVal( wxT("alpha"), wxT("0.0") ) ).ToDouble( &alpha );
                    }
                ...
                load( path, threshold, active, showFS, useTex, alpha );
        ...
        else if ( child->GetName() == wxT("selection_boxes") /*&& TheDataset::fibers_loaded*/)
        {
            ...
            while ( boxNode )
            {
                ...
                if ( _type == wxT("MASTER") )
                {
                    ...
                    currentMasterId = mainFrame->m_treeWidget->AppendItem( mainFrame->m_tSelBoxId,
                            selBox->getName(), 0, -1, selBox );
                ...
                else
                {
                    ...
                    wxTreeItemId boxId = mainFrame->m_treeWidget->AppendItem( currentMasterId,
                            selBox->getName(), 0, -1, selBox );
...
void DatasetHelper::save( const wxString filename )
{
    ...
    wxXmlProperty *rot00 =
            new wxXmlProperty( wxT("rot00"), wxString::Format( wxT("%.8f"), m_transform.s.M00 ) );
    wxXmlProperty *rot10 = new wxXmlProperty( wxT("rot10"),
            wxString::Format( wxT("%.8f"), m_transform.s.M10 ), rot00 );
    wxXmlProperty *rot20 = new wxXmlProperty( wxT("rot20"),
            wxString::Format( wxT("%.8f"), m_transform.s.M20 ), rot10 );
    wxXmlProperty *rot01 = new wxXmlProperty( wxT("rot01"),
            wxString::Format( wxT("%.8f"), m_transform.s.M01 ), rot20 );
    wxXmlProperty *rot11 = new wxXmlProperty( wxT("rot11"),
            wxString::Format( wxT("%.8f"), m_transform.s.M11 ), rot01 );
    wxXmlProperty *rot21 = new wxXmlProperty( wxT("rot21"),
            wxString::Format( wxT("%.8f"), m_transform.s.M21 ), rot11 );
    wxXmlProperty *rot02 = new wxXmlProperty( wxT("rot02"),
            wxString::Format( wxT("%.8f"), m_transform.s.M02 ), rot21 );
    wxXmlProperty *rot12 = new wxXmlProperty( wxT("rot12"),
            wxString::Format( wxT("%.8f"), m_transform.s.M12 ), rot02 );
    wxXmlProperty *rot22 = new wxXmlProperty( wxT("rot22"),
            wxString::Format( wxT("%.8f"), m_transform.s.M22 ), rot12 );
    ...
    for ( int i = 0; i < countPoints; ++i )
    {
        ...
        wxXmlProperty *propz = new wxXmlProperty( wxT("z"),
                wxString::Format( wxT("%f"), point->getCenter().z ) );
        wxXmlProperty *propy = new wxXmlProperty( wxT("y"),
                wxString::Format( wxT("%f"), point->getCenter().y ), propz );
        wxXmlProperty *propx = new wxXmlProperty( wxT("x"),
                wxString::Format( wxT("%f"), point->getCenter().x ), propy );
    ...
    for ( unsigned int i = boxes.size(); i > 0; --i )
    {
        for ( unsigned int j = boxes[i - 1].size(); j > 0; --j )
        {
            ...
            wxXmlProperty *propz = new wxXmlProperty( wxT("z"), wxString::Format( wxT("%f"),
                    currentBox->getCenter().z ) );
            wxXmlProperty *propy = new wxXmlProperty( wxT("y"), wxString::Format( wxT("%f"),
                    currentBox->getCenter().y ), propz );
            wxXmlProperty *propx = new wxXmlProperty( wxT("x"), wxString::Format( wxT("%f"),
                    currentBox->getCenter().x ), propy );
            ...
            propy = new wxXmlProperty( wxT("y"), wxString::Format( wxT("%f"), currentBox->getSize().y ),
                    propz );
            propx = new wxXmlProperty( wxT("x"), wxString::Format( wxT("%f"), currentBox->getSize().x ),
                    propy );
            ...
            wxXmlProperty *propactive = new wxXmlProperty( wxT("active"),
                    currentBox->getActive() ? wxT("yes") : wxT("no"), proptype );
            wxXmlProperty *propvisible = new wxXmlProperty( wxT("visible"),
                    currentBox->getVisible() ? wxT("yes") : wxT("no"), propactive );
            wxXmlProperty *propisbox = new wxXmlProperty( wxT("isBox"), currentBox->getIsBox() ? wxT("yes")
                    : wxT("no"), propvisible );
    ...
    for ( int i = 0; i < countTextures; ++i )
    {
        ...
        if ( info->getType() < Surface_ )
        {
            ...
            wxXmlProperty *propT = new wxXmlProperty( wxT("threshold"), wxString::Format( wxT("%.2f"),
                    info->getThreshold() ) );
            wxXmlProperty *propTA = new wxXmlProperty( wxT("alpha"), wxString::Format( wxT("%.2f"),
                    info->getAlpha() ), propT );
            wxXmlProperty *propA = new wxXmlProperty( wxT("active"), info->getShow() ? _T("yes") : _T("no"),
                    propTA );
            wxXmlProperty *propF = new wxXmlProperty( wxT("showFS"),
                    info->getShowFS() ? _T("yes") : _T("no"), propA );
            wxXmlProperty *propU = new wxXmlProperty( wxT("useTex"),
                    info->getUseTex() ? _T("yes") : _T("no"), propF );
            statusNode->AddProperty( propU );
        }
    }

    wxXmlProperty *propPosX = new wxXmlProperty( wxT("x"), wxString::Format( wxT("%d"),
            mainFrame->m_xSlider->GetValue() ) );
    wxXmlProperty *propPosY = new wxXmlProperty( wxT("y"), wxString::Format( wxT("%d"),
            mainFrame->m_ySlider->GetValue() ), propPosX );
    wxXmlProperty *propPosZ = new wxXmlProperty( wxT("z"), wxString::Format( wxT("%d"),
            mainFrame->m_zSlider->GetValue() ), propPosY );
            ...
            if ( boxes[i][j]->getShow() )
            {
                ...
                for ( int b = z1; b < z2; ++b )
                {
                    for ( int r = y1; r < y2; ++r )
                    {
                        for ( int c = x1; c < x2; ++c )
                        {
                            dst->at( b * rows * columns + r * columns + c ) = src->at( b * rows * columns + r
                                    * columns + c );
                        }
                    }
                }
