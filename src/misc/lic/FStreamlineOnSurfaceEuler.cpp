            ...
            edges(3), p0p1(3), p0p2(3)
{
    ...
    try
    {
        ...
        m_dh->printDebug( _T("interpolate vectors..."), LOGLEVEL_MESSAGE );
        ...
        m_dh->printDebug( _T("done"), LOGLEVEL_MESSAGE );
        ...
        FIndex& vertId1, FIndex& vertId2, double fwd, FArray& exit, FIndex& currCell, FIndex& nextCell)
{
    ...
    if (n1.isValid())
    {
        ...
        m_grid->getEdgeNeighbor(n1, k, neighs2);
        ...
        m_grid->getEdgeNeighbor(n1, (k + 2) % 3, neighs2);
        if (neighs2.size() && neighs2[0] != curr)
        {
            n3 = neighs2[0];
