...
void GCoptimization::specializeSmoothCostFunctor(const UserFunctor f) {
	...
	m_smoothcostFn = new UserFunctor(f);
        //mexPrintf( "After new UserFunctor()\n" );
        //mexPrintf( "m_smoothcostFn adress : %i", m_smoothcostFn );
...
{
        //mexPrintf( "In CGopt::giveDataEnergyInternal()" );
	...
	for ( SiteID i = 0; i < m_num_sites; i++ )
	{
		...
		eng = eng + EnergyType(tc->compute(i,m_labeling[i]));
	}
...
{
       // mexPrintf( "In CGopt::giveSmoothEnergyInternal()" );
	...
	for ( i = 0; i < m_num_sites; i++ )
	{
		...
		for ( n = 0; n < numN; n++ )
		{
			nSite = nPointer[n];
			if ( nSite < i ) eng = 
				eng + EnergyType(weights[n]*(sc->compute(i,nSite,m_labeling[i],m_labeling[nSite])));
		}
	}
