...
void MainCanvas::changeOrthoSize()
{
    ...
    if (m_view == mainView)
    {
        ...
        if (ratio > 1.0)
            ...
            orthoModY = ((int) (orthoSizeNormal * (1.0 + (1.0 - ratio)))
                    - orthoSizeNormal) / 2;
    }
        ...
        case mainView:
        {
                ...
                else if (wxGetKeyState(WXK_CONTROL) && m_dh->getPointMode())
                {
                    ...
                    if (m_hr.hit && (m_hr.picked <= sagittal))
                    {
                        ...
                        SplinePoint *point = new SplinePoint(getEventCenter(),
                                m_dh);
                        wxTreeItemId pId =
                                m_dh->mainFrame->m_treeWidget->AppendItem(
                                        m_dh->mainFrame->m_tPointId,
                                        wxT("point"), -1, -1, point);
                        point->setTreeId(pId);
            ...
            if (event.LeftIsDown())
            {
                ...
                else
                {
                    ...
                    Matrix4fSetRotationFromMatrix3f(&m_dh->m_transform,
                            &m_thisRot); // Set Our Final Transform's Rotation From This One
                }
                ...
                else
                {
                    if (event.Dragging() && m_hr.picked < 10)
                    {
                        ...
                        GetEventHandler()->ProcessEvent(event1);
                            ...
                            && m_hr.picked < 20)
                    {
                        ((SelectionBox*) m_hr.object)->processDrag(
                                event.GetPosition(), m_lastPos);
...
float MainCanvas::getAxisParallelMovement(int x1, int y1, int x2, int y2,
        Vector n)
{
...
hitResult MainCanvas::pick(wxPoint click)
{
    ...
    gluUnProject(winX, winY, 0, modelview, projection, viewport, &m_pos1X,
            &m_pos1Y, &m_pos1Z);
    gluUnProject(winX, winY, 1, modelview, projection, viewport, &m_pos2X,
            &m_pos2Y, &m_pos2Z);
    ...
    if (m_dh->pointMode)
    {
        ...
        id = m_dh->mainFrame->m_treeWidget->GetFirstChild(
                m_dh->mainFrame->m_tPointId, cookie);
        while (id.IsOk())
        {
            SplinePoint *point =
                    (SplinePoint*) (m_dh->mainFrame->m_treeWidget->GetItemData(
                            id));
            ...
            id = m_dh->mainFrame->m_treeWidget->GetNextChild(
                    m_dh->mainFrame->m_tPointId, cookie);
        }
    }
    return hr;
    ...
    switch (m_view)
    {
        case mainView:
        {
            if (m_dh->scheduledScreenshot)
            {
                ...
                fbo.getTexObject(1)->saveImageToPPM(
                        (m_dh->m_screenshotName).mb_str());
                fbo.deactivate();
            ...
            else
            {
                glMatrixMode(GL_PROJECTION);
                glLoadIdentity();
                glOrtho(-orthoModX, orthoSizeNormal + orthoModX, -orthoModY,
                        orthoSizeNormal + orthoModY, -500, 500);
...
{
    Vector dir(m_pos2X - m_pos1X, m_pos2Y - m_pos1Y, m_pos2Z - m_pos1Z);
    Vector center(m_pos1X + m_hr.tmin * dir.x, m_pos1Y + m_hr.tmin * dir.y,
...
void MainCanvas::OnChar(wxKeyEvent& event)
{
    ...
    switch (event.GetKeyCode())
    {
        case WXK_LEFT:
            if (wxGetKeyState(WXK_CONTROL))
            {
                ...
                m_dh->mainFrame->m_xSlider->SetValue(
                        wxMax(0, m_dh->mainFrame->m_xSlider->GetValue() - 1));
            break;
        case WXK_RIGHT:
            if (wxGetKeyState(WXK_CONTROL))
            {
        ...
        case WXK_DOWN:
            if (wxGetKeyState(WXK_CONTROL))
            {
                ...
                m_dh->mainFrame->m_ySlider->SetValue(
                        wxMax(0, m_dh->mainFrame->m_ySlider->GetValue() - 1));
            break;
        case WXK_UP:
            if (wxGetKeyState(WXK_CONTROL))
            {
        ...
        case WXK_PAGEDOWN:
            m_dh->mainFrame->m_zSlider->SetValue(
                    wxMax(0, m_dh->mainFrame->m_zSlider->GetValue() - 1));
            break;
        case WXK_PAGEUP:
            m_dh->mainFrame->m_zSlider->SetValue(
    ...
    m_dh->updateView(m_dh->mainFrame->m_xSlider->GetValue(),
            m_dh->mainFrame->m_ySlider->GetValue(),
