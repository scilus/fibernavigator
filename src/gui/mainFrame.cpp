...
EVT_MENU( MENU_FILE_SAVE_FIBERS_DMRI,                       MainFrame::OnSaveFibersDMRI  )
...
EVT_MENU( MENU_OPTIONS_RULER_TOOL,                          MainFrame::OnRulerTool              )
EVT_MENU( MENU_OPTIONS_RULER_TOOL_CLEAR,                    MainFrame::OnRulerToolClear         )
EVT_MENU( MENU_OPTIONS_RULER_TOOL_ADD,                      MainFrame::OnRulerToolAdd           )
EVT_MENU( MENU_OPTIONS_RULER_TOOL_DEL,                      MainFrame::OnRulerToolDel           )
...
EVT_MENU( MENU_OPTIONS_SAMPLING_OPTNO,                      MainFrame::OnSetSamplingNo          )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT1,                       MainFrame::OnSetSampling1           )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT2,                       MainFrame::OnSetSampling2           )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT3,                       MainFrame::OnSetSampling3           )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT4,                       MainFrame::OnSetSampling4           )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT5,                       MainFrame::OnSetSampling5           )
EVT_MENU( MENU_OPTIONS_SAMPLING_OPT6,                       MainFrame::OnSetSampling6           )
...
EVT_MENU( MENU_LIST_COLOR_DISTANCE,                         MainFrame::OnListMenuDistance    )
EVT_MENU( MENU_LIST_COLOR_MINDISTANCE,                      MainFrame::OnListMenuMinDistance )
EVT_MENU( MENU_LIST_COLOR_CURVATURE,                        MainFrame::OnColorWithCurvature  )
EVT_MENU( MENU_LIST_COLOR_TORSION,                          MainFrame::OnColorWithTorsion    )
EVT_MENU( MENU_LIST_GENERATE_FIBERVOLUME,                   MainFrame::OnGenerateFiberVolume )
    ...
    wxFrame( i_parent, i_id, i_title, i_pos, i_size, i_style )
{
	...
	m_textFibersFilter	   = new wxStaticText(this, ID_FIBERS_FILTER_SLIDER_TEXT,wxT("Fibers Length Filters"),wxDefaultPosition, wxSize( 148, 20 ), wxALIGN_CENTRE);
	m_textFibersFilter->SetBackgroundColour(*wxLIGHT_GREY);
	m_sliderFibersFilterMin   = new wxSlider( this, ID_FIBERS_FILTER_SLIDER1, 0, 0, 100, wxDefaultPosition, wxSize( 148,20 ), wxSL_HORIZONTAL | wxSL_AUTOTICKS );
	m_sliderFibersFilterMin->SetBackgroundColour(*wxLIGHT_GREY);
	m_sliderFibersFilterMin->Disable();
	m_sliderFibersFilterMax   = new wxSlider( this, ID_FIBERS_FILTER_SLIDER2, 100, 0, 100, wxDefaultPosition, wxSize( 148, 20 ), wxSL_HORIZONTAL | wxSL_AUTOTICKS );
	m_sliderFibersFilterMax->SetBackgroundColour(*wxLIGHT_GREY);
	m_sliderFibersFilterMax->Disable();
	m_textFibersSampling	  = new wxStaticText(this, ID_FIBERS_SAMPLING_SLIDER_TEXT,wxT("Fibers Subsampling"),wxDefaultPosition, wxSize( 148, 20 ), wxALIGN_CENTRE);
	m_textFibersSampling->SetBackgroundColour(*wxLIGHT_GREY);
	m_sliderFibersSampling    = new wxSlider( this, ID_FIBERS_SAMPLING_SLIDER, 0, 0, 200, wxDefaultPosition, wxSize( 148,20 ), wxSL_HORIZONTAL | wxSL_AUTOTICKS );
	m_sliderFibersSampling->SetBackgroundColour(*wxLIGHT_GREY);
	m_sliderFibersSampling->Disable();

	//ruler
	m_rulerDisplay = new wxTextCtrl(this, ID_RULER_DISPLAY,wxT("Ruler: 0.00mm (0.00mm) - Inactive"),wxDefaultPosition, wxSize( 296, 24 ), wxTE_LEFT | wxTE_READONLY);
	m_rulerDisplay->Disable();
	//m_rulerDisplay->SetForegroundColour(*wxCYAN);
	
	m_rulerDisplay->SetForegroundColour(wxColour(wxT("#00EEEE")));
	m_rulerDisplay->SetBackgroundColour(*wxBLACK);
	wxFont font = m_rulerDisplay->GetFont();
	font.SetPointSize(10);
	font.SetWeight(wxBOLD);
	m_rulerDisplay->SetFont(font);
	//m_rulerDisplay->SetBackgroundColour(*wxWhite);
	...
	m_treeSizer->AddSpacer(2);
	m_treeSizer->Add( m_treeWidget,           1, wxALL,                  1 );
	m_treeSizer->AddSpacer(2);
	m_treeSizer->Add( m_textFibersFilter,	  0, wxALL | wxALIGN_CENTER, 0 );
	m_treeSizer->Add( m_sliderFibersFilterMin,   0, wxALL | wxALIGN_CENTER, 0 );  
	m_treeSizer->Add( m_sliderFibersFilterMax,   0, wxALL | wxALIGN_CENTER, 0 );  
	m_treeSizer->Add( m_textFibersSampling,   0, wxALL | wxALIGN_CENTER, 0 );
	m_treeSizer->Add( m_sliderFibersSampling,   0, wxALL | wxALIGN_CENTER, 0 ); 
	...
	m_leftMainSizer->Add( m_rulerDisplay,   0, wxALL | wxALIGN_CENTER, 0 );
...
// COMMENT
///////////////////////////////////////////////////////////////////////////
void MainFrame::OnSaveFibersDMRI( wxCommandEvent& WXUNUSED(event) )
{
    if( ! m_datasetHelper->m_fibersLoaded )
        return;
    ...
    wxString caption         = wxT( "Choose a file" );
    wxString wildcard        = wxT( "dmri fiber files (*.fib)|*.fib|*.*|*.*" );
    wxString defaultDir      = wxEmptyString;
    wxString defaultFilename = wxEmptyString;
    wxFileDialog dialog( this, caption, defaultDir, defaultFilename, wildcard, wxSAVE );
    dialog.SetFilterIndex( 0 );
...
{
   wxTreeItemId l_selId = m_treeWidget->GetSelection();

   if( treeSelected( l_selId) == MASTER_OBJECT || treeSelected(l_selId) == CHILD_OBJECT)
   {
       SelectionObject* l_sel = (SelectionObject*) m_treeWidget->GetItemData(l_selId);

       l_sel->UseForDistanceColoring(!l_sel->IsUsedForDistanceColoring());
       ColorFibers();
   }
...
void MainFrame::OnRulerTool( wxCommandEvent& WXUNUSED(event) )
{
	...
	if (m_datasetHelper->m_isRulerToolActive){
		m_rulerDisplay->Enable();
	} else {
		m_rulerDisplay->Disable();
		m_rulerDisplay->AppendText(wxT(" - Inactive"));
	}
...
void MainFrame::OnAssignColor( wxCommandEvent& WXUNUSED(event) )
{
    ...
    if( treeSelected( l_selectionObjectTreeId ) == MASTER_OBJECT )
    {
        ...
        l_selectionObject->setColor( l_col);
        l_selectionObject->setIsDirty( true );
    }
    else
    {
...
// COMMENT
///////////////////////////////////////////////////////////////////////////
void MainFrame::OnSetSampling1( wxCommandEvent& WXUNUSED(event) )
{
	m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/2);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSampling2( wxCommandEvent& WXUNUSED(event) )
{
    m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()-m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/4);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSampling3( wxCommandEvent& WXUNUSED(event) )
{
    m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()-m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/10);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSampling4( wxCommandEvent& WXUNUSED(event) )
{
    m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()-m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/20);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSampling5( wxCommandEvent& WXUNUSED(event) )
{
    m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()-m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/50);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSampling6( wxCommandEvent& WXUNUSED(event) )
{
    m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()-m_datasetHelper->m_mainFrame->m_sliderFibersSampling->GetMax()/100);
    refreshAllGLWidgets();
}
void MainFrame::OnSetSamplingNo( wxCommandEvent& WXUNUSED(event) )
{
	m_datasetHelper->m_mainFrame->m_sliderFibersSampling->SetValue(0);
    refreshAllGLWidgets();
}
...
void MainFrame::refreshAllGLWidgets()
{
	...
	if (m_datasetHelper->m_isRulerToolActive){
		wxString sbString1 = wxString::Format( wxT("Ruler: %4.2fmm (%2.2fmm)" ), m_datasetHelper->m_rulerFullLength, m_datasetHelper->m_rulerPartialLength );
		m_rulerDisplay->SetValue(sbString1);	
	} 	
...
void MainFrame::OnRightClickListItem( wxListEvent& event )
{
    ...
    else if( l_info->getType() == FIBERS )
    {
        ...
        l_menu->Append(MENU_LIST_COLOR_DISTANCE,    _T("Color with Distance Anchoring") );
        l_menu->Append(MENU_LIST_COLOR_MINDISTANCE, _T("Color with Minimum Distance Anchoring") );
        l_menu->Append(MENU_LIST_COLOR_CURVATURE,   _T("Color with Curvature"));
        l_menu->Append(MENU_LIST_COLOR_TORSION,     _T("Color with Torsion"));
        l_menu->Append(MENU_LIST_GENERATE_FIBERVOLUME, _T("Generate Fiber Density Volume"));
...
{	
    ...
    if(i_selectionObject->getSelectionType() == CISO_SURFACE_TYPE)
    {
        o_menu->Append(MENU_VOI_FLIP_NORMALS, _T("Flip Normals"));
    }
...
void MainFrame::FillChildBoxMenu( SelectionObject* i_selectionObject, wxMenu* o_menu )
{
    ...
    if(i_selectionObject->getIsNOT())
    {
        o_menu->Append( MENU_VOI_TOGGLE_ANDNOT, _T( "Set Rule to AND" ) );
    }
    else
    {
        o_menu->Append( MENU_VOI_TOGGLE_ANDNOT, _T( "Set Rule to NOT" ) );
    }
...
void MainFrame::DefaultFillMenu( SelectionObject* i_selectionObject, wxMenu* o_menu )
{
    ...
    if(i_selectionObject->m_sourceAnatomy != NULL)
    {
        l_menuItem = o_menu->AppendCheckItem( 
            MENU_VOI_SET_AS_DISTANCE_ANCHOR, 
            _T( "Set as Distance Anchor"),
            wxEmptyString);

        l_menuItem->Check(i_selectionObject->IsUsedForDistanceColoring());
    }
...
void MainFrame::updateMenus()
{
	...
	l_oMenu->Check( l_oMenu->FindItem( _T( "Toggle Ruler Tool" ) ),  m_datasetHelper->m_isRulerToolActive  );
	...
	GetToolBar()->ToggleTool( MENU_OPTIONS_RULER_TOOL,           m_datasetHelper->m_isRulerToolActive );
    ...
    if( l_item != -1 )
    {
        ...
        if( l_info->getType() < RGB )
        {
            ...
            l_sMenu->Enable( l_sMenu->FindItem( _T( "New Distance Map + Surface" ) ), true );
            GetToolBar()->EnableTool(MENU_SURFACE_NEW_OFFSET, true );
            l_sMenu->Enable( l_sMenu->FindItem( _T( "New Distance Map" ) ), true );
            GetToolBar()->EnableTool(MENU_SURFACE_DIST_MAP, true );
        }
        if( l_info->getType() < MESH )
        {
        ...
        if( l_info->getType() == SURFACE )
        {
            l_sMenu->Enable( l_sMenu->FindItem( _T( "Flip Normal Direction" ) ), true );
            l_sMenu->Enable( l_sMenu->FindItem( _T( "Toggle Lic" ) ), m_datasetHelper->m_vectorsLoaded );
            l_sMenu->Check(  l_sMenu->FindItem( _T( "Toggle Lic" ) ), l_info->getUseLIC() );
            l_sMenu->Check(  l_sMenu->FindItem( _T( "Flip Normal Direction" ) ), ( m_datasetHelper->m_normalDirection < 0 ) );
            l_sMenu->Enable( l_sMenu->FindItem( _T( "Smooth Surface (Loop SubD)" ) ), true );
        }
    }
}
