...
class InfinitePlane
{
	...
	typedef FFixArray<Scalar, 3> Vector;

	InfinitePlane():distance(0){}
	InfinitePlane(const Vector &n, Scalar d): normal(n), distance(d){}
	InfinitePlane(const Vector &norm, const Vector& point)
	:normal(norm), distance(point*normal){}
	
	Scalar getOffset() const{ return distance; }
...
class GLFrustum
	...
	typedef FFixArray<Scalar,3> Point; // Type for points
	typedef FFixArray<Scalar,3> Vector; // Type for vectors
	typedef FFixArray<Scalar,4> HVector; // Type for homogeneous vectors
	typedef InfinitePlane<Scalar> Plane; // Type for planes
	//typedef Geometry::Box<Scalar,3> Box; // Type for axis-aligned boxes
	typedef BoundingBox<Scalar> Box;
