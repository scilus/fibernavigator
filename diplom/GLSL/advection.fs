...
void main()
{
  ...
  /*/ calculate position */
  vec2 advectedPosition= gl_TexCoord[3].st - ( tensorAdvectionScale * vec2(tensor.x / textureSizeW, tensor.y / textureSizeH));
  vec4 advectedNoise = texture2D(tex3, advectedPosition);

  gl_FragColor=(advectedNoise * (1.0-noiseBlend)) + (noise * noiseBlend);
